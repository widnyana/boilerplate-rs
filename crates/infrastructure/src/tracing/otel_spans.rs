// stolen from https://github.com/svix/svix-webhooks/blob/v0.83.1/server/svix-server/src/core/otel_spans.rs
...
/// An implementor of [`MakeSpan`] which creates `tracing` spans populated with
/// information about the request received by an `axum` web server.
#[derive(Clone, Copy)]
...
// matched_path.as_str().to_owned()         } else {
...
//             http.target = %request.uri().path_and_query().map_or("", |p|
// p.as_str()),             http.user_agent = %user_agent,
//             otel.kind = "server",
...
impl<B> OnResponse<B> for AxumOtelOnResponse {
	fn on_response(self, response: &http::Response<B>, latency: std::time::Duration, span: &tracing::Span) {
...
impl OnFailure<ServerErrorsFailureClass> for AxumOtelOnFailure {
	fn on_failure(&mut self, failure_classification: ServerErrorsFailureClass, _latency: std::time::Duration, span: &tracing::Span) {
